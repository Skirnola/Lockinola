...
function Navigation({ view, navigate, data, openSettings }: { view: View; navigate: (v: View) => void; data: LearningData; openSettings: () => void }) {
  ...
  return <Sidebar className="study-sidebar" collapsible="icon"><SidebarHeader className="brand-area"><div className="brand-row"><a className="brand" href="#today" onClick={() => navigate("Today")}><img className="brand-mark" src="/lockinola-mark.svg" alt="" width="32" height="32" /><span className="brand-name">lockinola</span></a><SidebarTrigger className="sidebar-nav-toggle" /></div></SidebarHeader><SidebarContent><div className="nav-label">WORKSPACE</div><SidebarMenu>{navigation.map(({ name, icon: Icon }, index) => <SidebarMenuItem key={name} className={index === 4 ? "nav-divider" : ""}><SidebarMenuButton tooltip={name} isActive={view === name} onClick={() => { navigate(name); setOpenMobile(false); }} className="nav-link"><Icon size={18} /><span>{name}</span>{name === "Japanese" && view !== name && <span className="daily-dot" />}{view === name && <span className="active-dot" />}</SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu><div className="chapter-card"><span className="eyebrow"><Flag size={14} /> TARGET</span><strong>{target}</strong><p>Graduate with a strong technical foundation and usable Japanese.</p><div className="chapter-line"><span /></div><span className="subtle">Stage 10 · private release</span></div></SidebarContent><SidebarFooter><button className="profile profile-button" onClick={openSettings}><span className="avatar">{initials || "IQ"}</span><div><strong>{data.profile.displayName}</strong><span>Profile & backup</span></div><Settings size={15} /><span className="profile-dot" /></button></SidebarFooter></Sidebar>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/api/access", { cache: "no-store" }).then(async response => {
      ...
      if (!cancelled) { setAccessStatus({ mode: "unavailable", authenticated: false, accessConfigured: false, readyForPrivateRelease: false, aiConfigured: false, aiDailyLimit: 20, runnerIsLoopback: false }); setAccessChecked(true); }
    });
